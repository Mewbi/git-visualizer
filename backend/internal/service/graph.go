...
func (s *Service) GetGraph(ctx context.Context, owner, name string) (*repository.Graph, error) {
	...
	return s.fetchAndCache(ctx, owner, name)
}
...
func (s *Service) GetUpdates(ctx context.Context, owner, name, since string) (*repository.UpdatesResponse, error) {
	...
	if len(newNodes) > 0 {
		graph, berr := s.git.BuildFullGraph(ctx, owner, name, localPath)
		if berr == nil {
...
func (s *Service) RefreshGraph(ctx context.Context, owner, name string) (*repository.Graph, error) {
	...
	graph, err := s.git.BuildFullGraph(ctx, owner, name, localPath)
	if err != nil {
...
func (s *Service) fetchAndCache(ctx context.Context, owner, name string) (*repository.Graph, error) {
	localPath, err := s.git.EnsureRepo(ctx, owner, name)
	if err != nil {
		return nil, s.wrapRepoError(err)
	}
